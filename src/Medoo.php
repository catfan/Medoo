...
class Medoo
{
    ...
    protected static $reConnectTime = 0;

    protected static $maxReConnectTime = 3;

    protected $breakReconnect = true;
    ...
    public function exec($query, $map = [])
    {
        ...
        if ($statement) {
            ...
            try {
                ...
                if ($this->isBreak($e)) {
                    if (self::$maxReConnectTime > self::$reConnectTime) {
                        self::$reConnectTime++;
                        $this->connect();
                        return $this->exec($query, $map);
                    }
    ...
    public function action($actions)
    {
        if (is_callable($actions)) {
            try {
                ...
                if ($this->isBreak($e)) {
                    if (static::$maxReConnectTime > static::$reConnectTime) {
                        static::$reConnectTime++;
                        $this->connect();
    ...
    protected function isBreak(PDOException $e)
    {
    ...
    public function error_handler($errno, $errmsg, $errfile = '', $errline = 0)
    {
        throw new \PDOException($errfile . ':' . $errline . ' ' . $errmsg, $errno);
    }
}