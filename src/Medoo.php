...
class Medoo
{
    ...
    public function __construct($options = null)
    {
        try {
            ...
            if (isset($options['dsn'])) {
                ...
                if (
                    isset($options['port']) && is_int($options['port'] * 1)
                ) {
                    $port = $options['port'];
                }
                ...
                case 'oracle':
                    $attr = [
                        ...
                        'dbname' => $options['server']
                            ?
                            '//' . $options['server'] . ($is_port ? ':' . $port
                                : ':1521') . '/' . $options['database_name']
                            :
                ...
                case 'sqlite':
                    $this -> pdo = new PDO(
                        'sqlite:' . $options['database_file'], null, null,
                        $this -> option
                    );
            ...
            if (
                in_array(
                    $this -> database_type,
                    ['mariadb', 'mysql', 'pgsql', 'sybase', 'mssql']
                )
    ...
    {
        preg_match(
            '/(^#)?([a-zA-Z0-9_]*)\.([a-zA-Z0-9_]*)(\s*\[JSON\]$)?/', $string,
            $column_match
        );

        if (isset($column_match[2], $column_match[3])) {
            return '"' . $this -> prefix . $column_match[2] . '"."'
                . $column_match[3] . '"';
        }
        ...
        foreach ($columns as $key => $value) {
            if (is_array($value)) {
                ...
                if (!empty($match['alias'])) {
                    $stack[] = $this -> columnQuote($match['column']) . ' AS '
                        . $this -> columnQuote($match['alias']);
        ...
        foreach ($data as $key => $value) {
            ...
            if (
                ...
                $wheres[] = 0 !== count(
                    ...
                    ?
                    '(' . $this -> dataImplode(
                        $value, $map, ' ' . $relation_match[1]
                    ) . ')'
                    :
                    '(' . $this -> innerConjunct(
                        $value, $map, ' ' . $relation_match[1], $conjunctor
                    ) . ')';
            } else {
                if (
                    is_int($key)
                    ...
                    $wheres[] = $this -> columnQuote($match[1]) . ' '
                        . $match['operator'] . ' ' . $this -> columnQuote(
                            $match[3]
                        );
                } else {
                    preg_match(
                        '/(#?)([a-zA-Z0-9_\.]+)(\[(?<operator>\>|\>\=|\<|\<\=|\!|\<\>|\>\<|\!?~|~!)\])?/i',
                        $key, $match
                    );
                    ...
                    if (isset($match['operator'])) {
                        ...
                        if ($operator === '<>' || $operator === '><') {
                            if ($type === 'array') {
                                ...
                                $wheres[] = '(' . $column . ' BETWEEN '
                                    . $map_key . 'a AND ' . $map_key . 'b)';

                                $data_type = (is_numeric($value[0])
                                    && is_numeric($value[1])) ? PDO::PARAM_INT
                                    : PDO::PARAM_STR;
                        ...
                        /**
                         * 在此处添加了 NOT(key LIKE val OR key1 LIKE val1 ...) 操作
                         * 操作符为：~!
                         */
                        ...
                        if ($operator === '~' || $operator === '!~'
                            ...
                            foreach ($value as $index => $item) {
                                ...
                                if ($operator === '!~') {
                                    $like_clauses[] = $column . ($operator
                                        === '!~' ? ' NOT' : '') . ' LIKE '
                                        . $map_key . 'L' . $index;
                                } else {
                                    $like_clauses[] = $column . ' LIKE '
                                        . $map_key . 'L' . $index;
                                }
                        ...
                        case 'array':
                            $wheres[] = $column . ' IN (' . $this -> arrayQuote(
                                    $value
                                ) . ')';
                            break;
    ...
    protected function whereClause($where, &$map)
    {
        ...
        if (is_array($where)) {
            ...
            $single_condition = array_diff_key(
                $where, array_flip(
                [
                    'AND', 'OR', 'GROUP', 'ORDER', 'HAVING', 'LIMIT', 'LIKE',
                    'MATCH'
                ]
            )
            );
            ...
            if (!empty($single_condition)) {
                $condition = $this -> dataImplode(
                    $single_condition, $map, ' AND'
                );
            ...
            if (!empty($where_AND)) {
                ...
                $where_clause = ' WHERE ' . $this -> dataImplode(
                        $where[$value[0]], $map, ' AND'
                    );
            }
            ...
            if (isset($where['MATCH'])) {
                ...
                if (is_array($MATCH)
                    && isset($MATCH['columns'], $MATCH['keyword'])
                ) {
                    $mode = '';
                    ...
                    $columns = implode(
                        array_map([$this, 'columnQuote'], $MATCH['columns']),
                        ', '
                    );
                    $map_key = $this -> mapKey();
            ...
            if (isset($where['GROUP'])) {
                ...
                if (is_array($GROUP)) {
                    ...
                    $where_clause .= ' GROUP BY ' . $this -> columnQuote(
                            $where['GROUP']
                        );
                }

                if (isset($where['HAVING'])) {
                    $where_clause .= ' HAVING ' . $this -> dataImplode(
                            $where['HAVING'], $map, ' AND'
                        );
                }
            }
            ...
            if (isset($where['ORDER'])) {
                ...
                if (is_array($ORDER)) {
                    ...
                    foreach ($ORDER as $column => $value) {
                        if (is_array($value)) {
                            $stack[] = 'FIELD(' . $this -> columnQuote($column)
                                . ', ' . $this -> arrayQuote($value) . ')';
                        } else {
                            if ($value === 'ASC' || $value === 'DESC') {
                                $stack[] = $this -> columnQuote($column) . ' '
                                    . $value;
                            } else {
                                if (is_int($column)) {
                                    $stack[] = $this -> columnQuote($value);
                    ...
                    $where_clause .= ' ORDER BY ' . $this -> columnQuote(
                            $ORDER
                        );
                }
                ...
                if (
                    isset($where['LIMIT'])
                    && in_array(
                        $this -> database_type, ['oracle', 'mssql']
                    )
                ) {
                    $LIMIT = $where['LIMIT'];

                    if (is_numeric($LIMIT)) {
                        $where_clause .= ' FETCH FIRST ' . $LIMIT
                            . ' ROWS ONLY';
                    }

                    if (
                        is_array($LIMIT) && is_numeric($LIMIT[0])
                        && is_numeric(
                            $LIMIT[1]
                        )
                    ) {
                        $where_clause .= ' OFFSET ' . $LIMIT[0]
                            . ' ROWS FETCH NEXT ' . $LIMIT[1] . ' ROWS ONLY';
                    }
                }
            }
            ...
            if (isset($where['LIMIT'])
                ...
                if (
                    ...
                    $where_clause .= ' LIMIT ' . $LIMIT[1] . ' OFFSET '
                        . $LIMIT[0];
                }
            }
        } else {
    ...
    protected function selectContext(
        $table, &$map, $join, &$columns = null, $where = null, $column_fn = null
    ) {
        preg_match(
            '/(?<table>[a-zA-Z0-9_]+)\s*\((?<alias>[a-zA-Z0-9_]+)\)/i', $table,
            $table_match
        );
        ...
        if (isset($table_match['table'], $table_match['alias'])) {
            ...
            $table_query = $table . ' AS ' . $this -> tableQuote(
                    $table_match['alias']
                );
        } else {
        ...
        if (
            isset($join_key[0]) && strpos($join_key[0], '[') === 0
        ) {
            $table_join = [];
            ...
            foreach ($join as $sub_table => $relation) {
                ...
                if ($match['join'] !== '' && $match['table'] !== '') {
                    ...
                    if (isset($match['alias'])) {
                        $table_name .= 'AS ' . $this -> tableQuote(
                                $match['alias']
                            ) . ' ';
                    }

                    $table_join[] = $join_array[$match['join']] . ' JOIN '
                        . $table_name . $relation;
                }
            }
        ...
        return 'SELECT ' . $column . ' FROM ' . $table_query
        ...
        foreach ($columns as $key => $value) {
            if (is_int($key)) {
                ...
                if (isset($map[1])) {
                    ...
                    case 'JSON':
                        $stack[$column_key] = json_decode(
                            $data[$column_key], true
                        );
                        break;
    ...
    public function select($table, $join, $columns = null, $where = null)
    {
        ...
        $query = $this -> exec(
            $this -> selectContext($table, $map, $join, $columns, $where), $map
        );
    ...
    public function insert($table, $datas)
    {
        ...
        foreach ($columns as $key) {
            $fields[] = $this -> columnQuote(
                preg_replace("/(^#|\s*\[JSON\]$)/i", '', $key)
            );
        }

        return $this -> exec(
            'INSERT INTO ' . $this -> tableQuote($table) . ' (' . implode(
                ', ', $fields
            ) . ') VALUES ' . implode(', ', $stack), $map
        );
    }
    ...
    public function update($table, $data, $where = null)
    {
        ...
        return $this -> exec(
            'UPDATE ' . $this -> tableQuote($table) . ' SET ' . implode(
                ', ', $fields
    ...
    public function delete($table, $where)
    {
        ...
        return $this -> exec(
            'DELETE FROM ' . $this -> tableQuote($table) . $this -> whereClause(
                $where, $map
            ), $map
        );
    }
    ...
    public function replace($table, $columns, $where = null)
    {
            ...
            foreach ($columns as $column => $replacements) {
                if (is_array($replacements[0])) {
                    foreach ($replacements as $replacement) {
                        ...
                        $replace_query[] = $this -> columnQuote($column)
                            . ' = REPLACE(' . $this -> columnQuote($column)
                            . ', ' . $map_key . 'a, ' . $map_key . 'b)';

                        $map[$map_key . 'a'] = [
                            $replacement[0], PDO::PARAM_STR
                        ];
                        $map[$map_key . 'b'] = [
                            $replacement[1], PDO::PARAM_STR
                        ];
                    }
                } else {
                    $map_key = $this -> mapKey();
        ...
        return $this -> exec(
            'UPDATE ' . $this -> tableQuote($table) . ' SET ' . $replace_query
            . $this -> whereClause($where, $map), $map
        );
    }
    ...
    public function get($table, $join = null, $columns = null, $where = null)
    {
        ...
        $query = $this -> exec(
            $this -> selectContext($table, $map, $join, $columns, $where)
            . ' LIMIT 1', $map
        );
    ...
    public function has($table, $join, $where = null)
    {
        ...
        $query = $this -> exec(
            'SELECT EXISTS(' . $this -> selectContext(
                $table, $map, $join, $column, $where, 1
            ) . ')', $map
        );
    ...
    public function count($table, $join = null, $column = null, $where = null)
    {
        ...
        $query = $this -> exec(
            $this -> selectContext(
                $table, $map, $join, $column, $where, 'COUNT'
            ), $map
        );
    ...
    public function max($table, $join, $column = null, $where = null)
    {
        ...
        $query = $this -> exec(
            $this -> selectContext($table, $map, $join, $column, $where, 'MAX'),
            $map
        );
    ...
    public function min($table, $join, $column = null, $where = null)
    {
        ...
        $query = $this -> exec(
            $this -> selectContext($table, $map, $join, $column, $where, 'MIN'),
            $map
        );
    ...
    public function avg($table, $join, $column = null, $where = null)
    {
        ...
        $query = $this -> exec(
            $this -> selectContext($table, $map, $join, $column, $where, 'AVG'),
            $map
        );
    ...
    public function sum($table, $join, $column = null, $where = null)
    {
        ...
        $query = $this -> exec(
            $this -> selectContext($table, $map, $join, $column, $where, 'SUM'),
            $map
        );
    ...
    public function id()
    {
        ...
        if ($type === 'oracle') {
            ...
            return $this -> pdo -> query('SELECT SCOPE_IDENTITY()')
                -> fetchColumn();
        } elseif ($type === 'pgsql') {
    ...
    public function info()
    {
        ...
        foreach ($output as $key => $value) {
            $output[$key] = @$this -> pdo -> getAttribute(
                constant('PDO::ATTR_' . $value)
            );
        }
